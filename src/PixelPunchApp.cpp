...
void PixelPunchApp::fileDrop(FileDropEvent event)
{
    ...
    mTransformUI.setShape(cinder::Rectf(0, 0, static_cast<float>(mSourceImage.getWidth()), static_cast<float>(mSourceImage.getHeight())));
    mTransformUI.center();
    mTransformUI.init();
...
void PixelPunchApp::validateResultImage()
{
    ...
    if (mSourceImage.getData() && !isValid)
    {
        ...
        int ms = static_cast<int>((t2-t1)*1000);
